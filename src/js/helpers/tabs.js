...
export default function(props) {
	...
	function tabHandle(tabButtons, tabContainer, tabContents) {
		...
		function setActiveAnchor(anchor) {
			tabContents.forEach((tab, ind) => {
				...
				if (!ANIM_PROP) {
					if (tabContent === anchor) {
						...
						tabButtons[ind].classList.remove('is-inactive');
					}
					else {
						tab.classList.remove('is-active');
						tabButtons[ind].classList.remove('is-active');
						tabButtons[ind].classList.add('is-inactive');
					}
				}
				else {
					const animSettings = {
			...
			if (!ANIM_PROP && onAnimComplete) onAnimComplete(tabContainer);
		}
	}
}
