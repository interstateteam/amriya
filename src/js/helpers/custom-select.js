...
export default (props) => {
	...
	document.querySelectorAll('.js-scroll').forEach(list => {
		Scrollbar.init(list)
		list.querySelectorAll('.scrollbar-track').forEach(track => {
			track.remove()
		});
	});
	...
	function createNewHTML(select) {
		...
		const dropdownHTML = `<div class="${classPrefix}" ${dropdownJsSelector.slice(1, -1)}>
			<div class="${classPrefix}__name" ${buttonJsSelector.slice(1, -1)}>
				<span>${title}</span>
				<svg xmlns="http://www.w3.org/2000/svg" width="12.7" height="7.1" viewBox="0 0 12.7 7.1"><path d="M6.4 7.1L0 .7.7 0l5.7 5.6L12 0l.7.7z"/></svg>
			</div>
			<div class="${classPrefix}__list" ${listJsSelector.slice(1, -1)}>
				<div class="${classPrefix}__container js-scroll">
					${dropdownOptionsHTML}
				</div>
			</div>
		</div>`;
